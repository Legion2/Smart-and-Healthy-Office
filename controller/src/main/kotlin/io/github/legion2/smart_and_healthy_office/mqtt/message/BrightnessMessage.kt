    ...
    companion object {
        fun from(message: Message): BrightnessMessage {
            return BrightnessMessage(message.tags.getOrElse("room") { throw IllegalArgumentException("Invalid message, missing tag room: $message") }
                    , (message.fields.getOrElse("brightness") { throw IllegalArgumentException("Invalid message, missing field brightness: $message") } as? Double
                    ?: throw IllegalArgumentException("Invalid message, brightness field must be of type double: $message")), message.timestamp)
        }
    }
}
