    ...
    companion object {
        fun from(message: Message): HumidityMessage {
            return HumidityMessage(message.tags.getOrElse("room") { throw IllegalArgumentException("Invalid message, missing tag room: $message") }
                    , ((message.fields.getOrElse("humidity") { throw IllegalArgumentException("Invalid message, missing field humidity: $message") } as? Number
                    ?: throw IllegalArgumentException("Invalid message, humidity field must be of type double: $message"))).toDouble(), message.timestamp)
        }
    }
}
