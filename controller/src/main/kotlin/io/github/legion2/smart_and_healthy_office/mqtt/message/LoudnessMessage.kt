    ...
    companion object {
        fun from(message: Message): LoudnessMessage {
            return LoudnessMessage(message.tags.getOrElse("room") { throw IllegalArgumentException("Invalid message, missing tag room: $message") }
                    , ((message.fields.getOrElse("loudness") { throw IllegalArgumentException("Invalid message, missing field loudness: $message") } as? Number
                    ?: throw IllegalArgumentException("Invalid message, loudness field must be of type double: $message"))).toDouble(), message.timestamp)
        }
    }
}